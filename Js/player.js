...
class Player {
	...
	update(){
		...
		if(this.ammo < 10){
			this.sAmmoTimer = time/fps;
			if(time/fps - this.bullttTime > 0.55){
				this.ammo ++;
				this.bullttTime = time/fps; 
			}
		}

		this.bAmmo = (time/fps - this.sAmmoTimer >= this.sAmmoTime);
	...
	newBullet(Id){
		...
		if(this.bAmmo) scr += 2;
		return new bullet(this.x + this.w/2 , this.y + this.h/2, this.angle.sin, this.angle.cos, scr, this.bAmmo, Id); 
	}
	...
	draw(){
		...
		for(let i = 0;i < this.ammo; i++){
			if(time/fps - this.sAmmoTimer >= (i+1) * this.sAmmoTime/10) ctx.fillStyle = "green";
		...
		drawText(this.score, this.x + this.w/2, this.y+this.h-3, this.score < 10 ? 20:15,"","black");
	}
